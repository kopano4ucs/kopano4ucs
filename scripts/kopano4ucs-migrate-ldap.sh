...
grep ^attributetype /usr/share/zarafa4ucs-schema/zarafa4ucs.schema -A1 | grep NAME | sed -e 's#.*NAME..##g' -e 's#.$##' | while read line; do
	...
	ldapsearch -h "$LDAP_HOST" -p "$LDAP_PORT" -D "$LDAP_USER" -b "cn=users,$(ucr get ldap/base)" -y "$LDAP_PASSWD_FILE" -o ldif-wrap=no -LLL -x $line=* dn $line | \
...
grep ^dn "$LDAP_LDIF".step2_attribute_data | sort | uniq | sed 's#dn: ##' | while read dn; do
	ldapsearch -h "$LDAP_HOST" -p "$LDAP_PORT" -D "$LDAP_USER" -b "cn=users,$(ucr get ldap/base)" -y "$LDAP_PASSWD_FILE" -o ldif-wrap=no -LLL -x -s base -b "$dn" objectclass | \
