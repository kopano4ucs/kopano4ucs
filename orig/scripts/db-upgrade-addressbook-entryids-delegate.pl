...
print "Converting delegates...\n";

$query = 'SELECT 0,hierarchyid,orderid,val_binary FROM mvproperties WHERE type=4354 AND tag=26693 AND substr(val_binary,1,20) = 0x00000000AC21A95040D3EE48B319FBA753304425';
$upd = 'UPDATE mvproperties SET val_binary=? WHERE hierarchyid=? AND orderid=? AND type=4354 AND tag=26693';
...
while(@row = $sth->fetchrow_array()) {
	...
	print "Converting delegate $i\n";
	my $entryid = upgradeentryid($dbh, \%cache, $row[3]);
	if (defined($entryid)) {
		$udh->execute($entryid, $row[1], $row[2])
			or die $DBI::errstr;
	}
	$i++;
}
